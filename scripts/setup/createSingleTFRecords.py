...
ex = sacred.Experiment(name="generate_single_tfrecords_from_cached_clean")
...
def config():
    ...
    # new_train_index="/n/scratch2/ms994/medium_size"
    num_shards = 4
    overlap=2
    unit_size=4
    max_size=4
...
@ex.capture
def get_data_file_reader(cachedIndex, split, directory, overlap, unit_size, max_size, ):
    return ppv2.FileDataReader(split=split, directory=directory, cachedIndex=cachedIndex, overlap=overlap, unit_size=unit_size, max_size=max_size)
...
@ex.capture
def grab_datasets_files(dataset, indexDict, generator_func, split, num_shards=1):
    dataset_file_pairs = []
...
def main():
    ...
    train = ppv2.FileDataReader(directory="/n/scratch2/ms994/medium_size/train", split="train", train_label_files_segs=trainFiles, filename="/home/ms994/train_4s.pkl", max_size=4)
    valid = ppv2.FileDataReader(directory="/n/scratch2/ms994/medium_size/valid", split="valid", train_label_files_segs=validFiles, filename="/home/ms994/valid_4s.pkl", max_size=4)
    test = ppv2.FileDataReader(directory="/n/scratch2/ms994/medium_size/test", split="test", train_label_files_segs=testFiles, filename="/home/ms994/test_4s.pkl", max_size=4)

    grab_datasets_files(train, train.indexDict, train_generator, "train_4s")
    grab_datasets_files(valid, valid.indexDict, valid_generator, "valid_4s")
    grab_datasets_files(test, test.indexDict, test_generator, "test_4s")
