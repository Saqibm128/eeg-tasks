...
# Custom loss layer
class TFHomeoschedasticMultiLossLayer(tf.keras.layers.Layer):
    def __init__(self, nb_outputs=2, loss_funcs=[], multiplier=None, **kwargs):
        #multiplier is used to change signs
        self.nb_outputs = nb_outputs
        self.output_dim = nb_outputs
        ...
        if multiplier is None:
          ...
          self.multiplier = multiplier
        self.loss_funcs = loss_funcs
        super(TFHomeoschedasticMultiLossLayer, self).__init__(**kwargs)
    ...
    def build(self, input_shape=None):
        ...
        for i in range(self.nb_outputs):
            ...
            self.log_vars += [self.add_weight(name='log_var' + str(i), shape=(1,),
                                              initializer=initializer, trainable=True)]
        super(TFHomeoschedasticMultiLossLayer, self).build(input_shape)
    ...
    def multi_loss(self, ys_true, ys_pred):
        ...
        for i, zipped_args in enumerate(zip(ys_true, ys_pred, self.log_vars)):
            ...
            precision = tf.keras.backend.exp(-log_var[0])
            sign_cost = 1 if self.multiplier[i] > 0 else -1
            if self.multiplier[i] == 0:
                sign_cost = 0
            try:
                loss += tf.keras.backend.sum(precision  * self.loss_funcs[i](y_true, y_pred) +   log_var[0], -1)
            except:
                loss += (precision  * self.loss_funcs[i](y_true, y_pred) +   log_var[0])
    ...
    def call(self, inputs):
        ...
        self.add_loss(loss, inputs=inputs)
        # pass thru the predictions
        return tf.keras.backend.concatenate(ys_pred)
