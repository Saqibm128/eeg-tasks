...
do
  ...
  do
    ...
    do
      sbatch -n 1 --mem-per-cpu 16G -t 4:00:00 -p gpu --gres=gpu:1 run.sh standardized_combined_simple_ensemble run_on_training_loss batch_size=32 n_process=1 lr=0.0002 use_vp=False num_conv_spatial_layers=$num_conv_spatial_layers patience=5 num_conv_temporal_layers=$num_conv_temporal_layers $conv_spatial_filter num_epochs=50 num_spatial_filter=$num_spatial_filter
    done
  done
done
...
do
  ...
  do
    ...
    do
      ...
      do
      sbatch -n 1 --mem-per-cpu 16G -t 1:00:00 -p gpu --gres=gpu:1 run.sh steps_per_epoch=$num_steps_per_epoch standardized_combined_simple_ensemble batch_size=32 n_process=1 lr=0.0002 use_vp=False num_conv_spatial_layers=$num_conv_spatial_layers num_conv_temporal_layers=$num_conv_temporal_layers $conv_spatial_filter num_spatial_filter=$num_spatial_filter
    done
  done
  done
done
